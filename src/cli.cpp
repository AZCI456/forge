...
int run_review_command(const std::vector<std::string> & command_flags) {
    ...
    // can change around the commands
    system_command = "cat >> REVIEW.MD";

    // run the command
    std::cout << "--------------" << std::endl;
    if (! command_flags.empty() && command_flags[0] == "-v") { // handle subcommands
        // keeping this over system command as for something as trivial as printing document
        // contents we shouldn't span a separate shell process
        printFileContent("REVIEW.MD");
        return 0;
    }
    else if (std::system(system_command.c_str()) != 0) {
 ...
 * was caused by stale pointers which might have come about b/c when system
 ...
 *
 * looks cooler tho with the "> " old school additions
     * NOTE: currently the main issue is when the paragraph gets too long and exceeds the line length
     * it doesn't wrap around it just at the end of it adds another "> "
     * which isn't great. Probably could call a line with it so it's on a new line but still makes writing a.
     * full continuous paragraph disjoined
 *
 * @param command_flags
 * @return
 */
